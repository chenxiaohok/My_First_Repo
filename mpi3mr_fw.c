	...
	struct mpi3_default_reply *def_reply)
{
	...
	//host_tag 这个不就是smid吗？
	switch (host_tag) {
	case MPI3MR_HOSTTAG_INITCMDS:				//初始化的
		return &mrioc->init_cmds;
	case MPI3MR_HOSTTAG_BLK_TMS:				//处理task management的
		return &mrioc->host_tm_cmds;
	case MPI3MR_HOSTTAG_INVALID:				//无效
	...
	*reply_dma = 0;
	...
	case MPI3_REPLY_DESCRIPT_FLAGS_TYPE_ADDRESS_REPLY:
		...
		*reply_dma = le64_to_cpu(addr_desc->reply_frame_address);
		...
		ioc_status &= MPI3_REPLY_DESCRIPT_STATUS_IOCSTATUS_STATUS_MASK;
		//如果是scsi io出现的错误 获取sense buf的虚拟地址
...
static int mpi3mr_process_admin_reply_q(struct mpi3mr_ioc *mrioc)
{
	...
	//Admin reply queue consumer index
	...
	do {
		//Admin request queue consumer index 索引 用来取请求描述符的索引
	...
	struct mpi3mr_intr_info *intr_info)
{
	...
	//Expected phased identifier for the reply queue
	...
	//&操作后 值要么为1 要么为0 很明显 当该值为!= exp_phase 退出
	if ((le16_to_cpu(reply_desc->reply_flags) &
	    MPI3_REPLY_DESCRIPT_FLAGS_PHASE_MASK) != exp_phase) {
	...
	do {
		//获取队列id 因为队列id从1开始 所以减1 获取队列的下标
		req_q_idx = le16_to_cpu(reply_desc->request_queue_id) - 1;

		// Operational request queue info pointer
		//获取结构
		...
		//reply_qidx = op_reply_q->qid - 1;是回复队列的下标
		//回复描述符处理
...
static int mpi3mr_setup_isr(struct mpi3mr_ioc *mrioc, u8 setup_one)
{
	...
	//Operational reply queue offset with MSIx  要么0 要么1
	...
	u16 admin_req_sz, u8 ignore_reset)
{
	...
	//取请求帧 注意mrioc->admin_req_base 这个确实是用来存放请求帧的空间 dma空间
...
static int mpi3mr_alloc_op_reply_q_segments(struct mpi3mr_ioc *mrioc, u16 qidx)
{
	...
	if (mrioc->enable_segqueue) {
		...
		//Depth of each segments
		op_reply_q->segment_qd =
		    MPI3MR_OP_REP_Q_SEG_SIZE / mrioc->op_reply_desc_sz;		//4096/

		//回复的段的大小
		size = MPI3MR_OP_REP_Q_SEG_SIZE;

		//Segment list base virtual address
		...
		//Depth of each segments op_reply_q->num_replies默认是4096个
...
static int mpi3mr_create_op_reply_q(struct mpi3mr_ioc *mrioc, u16 qidx)
{
	...
	//取 Queue Id starting from 1
	reply_qid = op_reply_q->qid; 			//0
	...
	reply_qid = qidx + 1;		//1-23

	//Maximum number of entries in the queue
	//？暂且先认为是回复描述符的数量
	op_reply_q->num_replies = MPI3MR_OP_REP_Q_QD;				//4096
	...
	//0-22+1 1-23
	op_reply_q->qid = reply_qid;			//从1开始

	//0-22+1 1-23				此时 midx与qid一致 
	...
	u16 reply_qid)
{
	...
	op_req_q->num_requests = MPI3MR_OP_REQ_Q_QD;		//512
	op_req_q->ci = 0;
	//producer index
	op_req_q->pi = 0;
...
static int mpi3mr_create_op_queues(struct mpi3mr_ioc *mrioc)
{
	...
	//Operational reply queue offset with MSIx  	assmed 23
	msix_count_op_q =
	    mrioc->intr_info_count - mrioc->op_reply_q_offset;

	//Number of operational queues
	if (!mrioc->num_queues)
	...
	if (!mrioc->req_qinfo) {
		...
		//Operational reply queue info pointer
	...
	//Number of operational reply queues 操作队列数量=请求队列数量=i 23
...
static int mpi3mr_setup_admin_qpair(struct mpi3mr_ioc *mrioc)
{
	...
	//Admin request queue size  应该是请求帧池的大小
	mrioc->admin_req_q_sz = MPI3MR_ADMIN_REQ_Q_SIZE;				//8192

	//Number of admin requests 
	//应该是请求帧的个数
	mrioc->num_admin_req = mrioc->admin_req_q_sz /					//8192/128=64
	    MPI3MR_ADMIN_REQ_FRAME_SZ;

	//Admin request queue consumer index
	mrioc->admin_req_ci = mrioc->admin_req_pi = 0;					//用于获取请求帧的下标

	//Admin request queue base virtual address
	mrioc->admin_req_base = NULL;			

	// Admin reply queue size 回复振帧的大小
	mrioc->admin_reply_q_sz = MPI3MR_ADMIN_REPLY_Q_SIZE;		//8192	

	//Admin reply queue size 	回复帧的数目
	mrioc->num_admin_replies = mrioc->admin_reply_q_sz /
	    MPI3MR_ADMIN_REPLY_FRAME_SZ;

	//Admin reply queue consumer index	
	mrioc->admin_reply_ci = 0;									//用于获取回复帧的下标							

	//Admin reply queue expected phase
...
static int mpi3mr_alloc_reply_sense_bufs(struct mpi3mr_ioc *mrioc)
{
	...
	//Device handle bitmap size
	mrioc->dev_handle_bitmap_sz = mrioc->facts.max_devhandle / 8;
	if (mrioc->facts.max_devhandle % 8)
		mrioc->dev_handle_bitmap_sz++;
	//Remove pending bitmap
	mrioc->removepend_bitmap = kzalloc(mrioc->dev_handle_bitmap_sz,
	    GFP_KERNEL);
	if (!mrioc->removepend_bitmap)
		goto out_failed;

	//Device removal bitmap size
	...
	//Number of reply buffers allocated 最大请求数加上一些event事件
	mrioc->num_reply_bufs = mrioc->facts.max_reqs + MPI3MR_NUM_EVT_REPLIES;	
	//深度 
	mrioc->reply_free_qsz = mrioc->num_reply_bufs + 1;

	//Number of sense buffers
...
static int mpi3mr_alloc_chain_bufs(struct mpi3mr_ioc *mrioc)
{
	...
	sz = MPI3MR_PAGE_SIZE_4K;			//4096 4K
...
int mpi3mr_init_ioc(struct mpi3mr_ioc *mrioc, u8 init_type)
{
	...
	//usleep unit used in threaded isr irqpoll
	...
	//上述操作 主要是将ioc进行复位 保证状态为复位状态
	//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

	// Setup admin queue pair  分配请求 回复帧池
