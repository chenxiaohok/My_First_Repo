	...
	struct mpi3_default_reply_descriptor *reply_desc, u64 *reply_dma, u16 qidx)
{
	...
	*reply_dma = 0;
	...
	//跟踪消息
	trace_mpi(mrioc,(union mpi3_reply_descriptors_union *)reply_desc,qidx);
	...
	case MPI3_REPLY_DESCRIPT_FLAGS_TYPE_ADDRESS_REPLY:
		...
		*reply_dma = le64_to_cpu(addr_desc->reply_frame_address);
		...
		xfer_count = le32_to_cpu(scsi_reply->transfer_count);			//传输的内容
...
mpi3mr_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	//初始值为0
	mrioc->logging_level = logging_level;		//0
	mrioc->shost = shost;
	mrioc->pdev = pdev;
